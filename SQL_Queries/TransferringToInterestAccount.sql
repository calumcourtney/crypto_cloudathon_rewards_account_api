	DO $$
	DECLARE chain_symbol_var VARCHAR ( 255 );
	...
	BEGIN
	  ...
	  where supported_chains.symbol = chain_symbol_var and users.user_id = user_id_var);
	  RAISE NOTICE 'TEST2';
	  ...
	  if interest_account_id_var IS NULL then
		  ...
		  interest_account_supported_coins_id_var := (select interest_account_supported_coins_id from interest_account_supported_coins inner join supported_chains ON interest_account_supported_coins.chain_id=supported_chains.chain_id where supported_chains.symbol = chain_symbol_var);
